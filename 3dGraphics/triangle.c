	...
	text2_t a_uv, text2_t b_uv, text2_t c_uv)
{
	...
	int text_index = (tex_y * texture_width + tex_x) % (texture_width * texture_height);

	//Adjust 1/w so the pixels that are closer to tha camera have smaller values
	interpolated_reciprocal_w = 1.0 - interpolated_reciprocal_w;

	//Only draw the pixel if the depth value is less 
	if (interpolated_reciprocal_w < z_buffer[(window_width * y) + x])
	{
		draw_pixel(x, y, texture[text_index]);
